...
/** 分段编辑器的分页数据。total 为当前筛选条件命中的总段数（分页器用）；stats 为全会话口径。
 *  prevRowId 无上一段时为 -1；chapter 未分章为空串；planType 为 IMPORT/OVERWRITE/SKIP。 */
public record ImportRowsPageVO(ImportEditStatsVO stats, int page, int totalPages, long total, List<RowVO> rows) {
